...
For this static build profile, `lua-nginx-module` resty-core auto-load failure is made non-fatal during VM initialization, and modules are resolved through `lua_package_path` at runtime.

When using custom nginx configurations, `lua_package_path` SHOULD include `${prefix}/lib/lua/?.lua` so `resty.core` can be resolved.
